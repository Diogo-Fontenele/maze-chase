...
export default function App() {
  ...
  const [connOk, setConnOk]     = useState(null) // null=checking, true=ok, false=error
  ...
  useEffect(() => {
    ...
    canvas.width  = MW * CELL
    canvas.height = MH * CELL
  ...
  const enterRoom = useCallback(async () => {
    ...
    if (!s) { setMsg('❌ Sala não encontrada.'); return }
    if (s.phase !== 'lobby') { setMsg('❌ Partida já em andamento!'); return }
    if (Object.keys(s.players).length >= 10) { setMsg('❌ Sala cheia!'); return }
    if (s.players[pid]) { setGs(s); setJoined(true); setScreen('lobby'); setMsg(''); return }
    const nt = Object.values(s.players).filter(p=>p.role==='thief').length
    if (roleChoice==='thief' && nt>=4) { setMsg('⚠️ Já tem 4 ladrões! Escolha Policial.'); return }
    ...
    const ok = await roomSave(roomCode, s)
    if (!ok) { setMsg('❌ Erro ao salvar. Tente novamente.'); return }
  ...
  const startGame = useCallback(async () => {
    ...
    if (!all.some(p=>p.role==='cop'))   { setMsg('⚠️ Precisa de pelo menos 1 policial!'); return }
    if (!all.some(p=>p.role==='thief')) { setMsg('⚠️ Precisa de pelo menos 1 ladrão!');  return }
    const maze = buildMaze(MW, MH)
    ...
    <div style={S.root}>
      <div style={S.card}>
        ...
        <div style={{display:'flex',gap:8,width:'100%'}}>
          <input style={{...S.inp,flex:1,textTransform:'uppercase',letterSpacing:6,fontSize:18,textAlign:'center'}}
  ...
  if (screen==='lobby') return (
    <div style={S.root}>
      <div style={{...S.card,width:480}}>
        ...
        <p style={{color:'#4a8aaa',fontSize:11}}>Compartilhe o código <strong style={{color:'#00c9ff',letterSpacing:3}}>{roomCode}</strong> com seu filho</p>
        ...
        <div style={{position:'relative',flexShrink:0}}>
          ...
          {me.caught && (
            <div style={{position:'absolute',inset:0,background:'#000c',display:'flex',
              ...
              🔒<br/>PRESO<br/><span style={{fontSize:13,color:'#888'}}>Aguarde o fim...</span>
            </div>
          )}
        </div>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:10,width:178}}>
          ...
          <div style={{...S.panel,flex:1}}>
            ...
            <div style={{display:'flex',gap:4}}>
              ...
              <button style={{...S.btnP,padding:'3px 10px',width:'auto',fontSize:12,letterSpacing:0}}
...
const S = {
  ...
  badge:{ display:'inline-block',borderRadius:20,padding:'3px 10px',fontSize:12,fontWeight:700,margin:'2px 3px' },
  sbar:{ fontSize:12,padding:'5px 14px',borderRadius:20,border:'1px solid transparent',width:'100%',textAlign:'center' },
  ...
  divider:{ width:'100%',display:'flex',alignItems:'center',
    borderTop:'1px solid #1a3a55',margin:'4px 0' },
