...
async function roomLoad(code){
  ...
  if(error){
    if(error.code!=='PGRST116') console.error('[roomLoad]',error)
    return null
  }
...
// ─── Debug log ────────────────────────────────────────────────────────────────
let _debugLog=[]
function dbg(status,msg,detail=''){
  const entry={status,msg,detail,time:new Date().toLocaleTimeString()}
  _debugLog=[..._debugLog.slice(-19),entry]
...
export default function App(){
  ...
  const log=useCallback((status,msg,detail='')=>{
    _debugLog=dbg(status,msg,detail)
    setDebugLines([..._debugLog])
  },[])

  // ── Connection test on mount ───────────────────────────────────────────────
  useEffect(()=>{
    const run=async()=>{
      log('⏳','Iniciando testes de conexão...')

      // Step 1: Check supabase key format
      log('⏳','[1/5] Verificando formato da chave...')
      try{
        const {SUPABASE_URL,SUPABASE_KEY}=await import('./supabase').then(m=>({
          SUPABASE_URL:m.supabase.supabaseUrl,
          SUPABASE_KEY:m.supabase.supabaseKey
        })).catch(()=>({SUPABASE_URL:'?',SUPABASE_KEY:'?'}))
        const url=supabase.supabaseUrl||'?'
        const key=supabase.supabaseKey||''
        const isJWT=key.split('.').length===3&&key.startsWith('eyJ')
        const isPub=key.startsWith('sb_publishable_')
        if(isPub){
          log('❌','[1/5] Chave inválida!',
            `Formato "sb_publishable_" NÃO é suportado pelo supabase-js.\nVá em: Supabase → Project Settings → API → "anon" key (começa com eyJ...)`)
        } else if(isJWT){
          log('✅','[1/5] Chave JWT válida',url)
        } else {
          log('⚠️','[1/5] Chave com formato desconhecido',key.slice(0,20)+'...')
        }
      }catch(e){
        log('❌','[1/5] Erro ao verificar chave',e.message)
      }

      // Step 2: Network - can we reach Supabase at all?
      log('⏳','[2/5] Testando conectividade de rede...')
      try{
        const url=supabase.supabaseUrl||'https://wlglksboakpsibgykymx.supabase.co'
        const r=await fetch(url+'/rest/v1/',{
          headers:{apikey:supabase.supabaseKey||'','Content-Type':'application/json'},
          signal:AbortSignal.timeout(5000)
        })
        log(r.ok||r.status===400?'✅':'⚠️','[2/5] Rede OK - Supabase respondeu',`HTTP ${r.status}`)
      }catch(e){
        log('❌','[2/5] Sem acesso ao Supabase',e.message)
      }

      // Step 3: Auth - can we query?
      log('⏳','[3/5] Testando autenticação...')
      try{
        const {error}=await supabase.from('rooms').select('code').limit(1)
        if(error) log('❌','[3/5] Erro de autenticação',`${error.code}: ${error.message}`)
        else log('✅','[3/5] Auth OK - Tabela rooms acessível')
      }catch(e){
        log('❌','[3/5] Exceção ao autenticar',e.message)
      }

      // Step 4: Write - can we insert/upsert?
      log('⏳','[4/5] Testando escrita na tabela...')
      try{
        const testCode='__test__'
        const {error}=await supabase.from('rooms')
          .upsert({code:testCode,state:{test:true},updated_at:new Date().toISOString()},{onConflict:'code'})
        if(error) log('❌','[4/5] Erro ao escrever',`${error.code}: ${error.message}\nHint: ${error.hint||'—'}`)
        else log('✅','[4/5] Escrita OK')
      }catch(e){
        log('❌','[4/5] Exceção ao escrever',e.message)
      }

      // Step 5: Realtime
      log('⏳','[5/5] Testando Realtime...')
      try{
        const ch=supabase.channel('test-ping')
        await new Promise((res,rej)=>{
          const t=setTimeout(()=>rej(new Error('Timeout 5s')),5000)
          ch.subscribe(status=>{
            clearTimeout(t)
            if(status==='SUBSCRIBED') res(status)
            else rej(new Error('Status: '+status))
          })
        })
        supabase.removeChannel(ch)
        log('✅','[5/5] Realtime OK')
      }catch(e){
        log('⚠️','[5/5] Realtime falhou',e.message)
      }
  ...
  const createRoom=useCallback(async()=>{
    ...
    log('⏳','Criando sala...','Código: '+code)
    try{
      setLoader({pct:30,label:'Preparando sala...'})
      log('⏳','Gerando labirinto e joias...','')
      await new Promise(r=>setTimeout(r,200))
      setLoader({pct:65,label:'Salvando no servidor...'})
      log('⏳','Salvando no Supabase...','')
      await roomSave(code,state)
      setLoader({pct:100,label:'Pronto!'})
      log('✅','Sala criada!','Código: '+code)
      await new Promise(r=>setTimeout(r,300))
      setLoader(null)
      setRoomCode(code);setGs(state);setMsg('')
      subscribe(code);setScreen('join')
    }catch(e){
      setLoader(null)
      log('❌','Falha ao criar sala',e.message)
      setMsg('❌ '+e.message)
    }
  },[subscribe,log])
  ...
  const joinByCode=useCallback(async()=>{
    ...
    setMsg('');log('⏳','Entrando na sala '+code)
    try{
      setLoader({pct:40,label:'Buscando sala...'})
      const s=await roomLoad(code)
      if(!s){
        setLoader(null)
        log('❌','Sala não encontrada','Código: '+code)
        setMsg('❌ Sala "'+code+'" não encontrada.')
        return
      }
      setLoader({pct:100,label:'Conectado!'})
      log('✅','Sala encontrada','Código: '+code)
      await new Promise(r=>setTimeout(r,300))
      setLoader(null)
      ...
      setLoader(null)
      log('❌','Erro ao entrar na sala',e.message)
      setMsg('❌ '+e.message)
    }
  },[joinInput,subscribe,log])

  // ── Enter as player ────────────────────────────────────────────────────────
  const enterRoom=useCallback(async()=>{
    const name=nameInput.trim()
    if(!name){setMsg('⚠️ Digite seu nome!');return}
    setMsg('');log('⏳','Registrando jogador: '+name)
    try{
      setLoader({pct:40,label:'Verificando sala...'})
      ...
      if(s.players[pid]){setGs(s);setMsg('');setScreen('lobby');setLoader(null);return}
      ...
      setLoader({pct:80,label:'Salvando...'})
      await roomSave(roomCode,s)
      setLoader({pct:100,label:'Entrou!'})
      log('✅','Jogador registrado','Nome: '+name+' | Papel: '+roleChoice)
      await new Promise(r=>setTimeout(r,300))
      setLoader(null)
      setGs(s);setMsg('');setScreen('lobby')
    }catch(e){
      setLoader(null)
      log('❌','Erro ao registrar jogador',e.message)
      setMsg('❌ '+e.message)
    }
  },[nameInput,roleChoice,roomCode,pid,log])
  ...
  const changeRole=useCallback(async(nr)=>{
    try{
      ...
      s.players[pid]={...s.players[pid],role:nr,x:sx,y:sy,color:nr==='thief'?TC[ntNew%4]:CC[ri%6]}
      await roomSave(roomCode,s)
      setGs(s);setMsg('')
  ...
  const startGame=useCallback(async()=>{
    ...
    try{
      setLoader({pct:20,label:'Verificando times...'})
      ...
      s.phase='playing'
      setLoader({pct:90,label:'Iniciando...'})
      await roomSave(roomCode,s)
      ...
      setLoader(null)
      log('❌','Erro ao iniciar partida',e.message)
      setMsg('❌ '+e.message)
    }
  },[roomCode,log])
  ...
  const doMove=useCallback(async(dx,dy)=>{
    ...
    try{
      ...
      const th=Object.values(s.players).filter(p=>p.role==='thief')
      if(th.every(p=>p.caught)){s.phase='gameover';s.winner='cops'}
  ...
  const DebugPanel=()=>(
    <div style={{
      ...
      background:'#020c14',border:'1px solid #1a3a55',
      borderRadius:10,overflow:'hidden',fontFamily:'monospace'
    }}>
      <div style={{
        display:'flex',justifyContent:'space-between',alignItems:'center',
        background:'#0a1929',padding:'6px 12px',borderBottom:'1px solid #1a3a55'
      }}>
        <span style={{color:'#00c9ff',fontSize:12,fontWeight:700}}>🔍 DEBUG — Diagnóstico</span>
        <div style={{display:'flex',gap:8,alignItems:'center'}}>
          <span style={{color:'#4a8aaa',fontSize:11}}>🌐 {online} online</span>
          <button onClick={()=>setShowDebug(v=>!v)} style={{
            background:'transparent',border:'1px solid #1a3a55',borderRadius:4,
            color:'#4a8aaa',fontSize:11,cursor:'pointer',padding:'2px 8px'
          }}>{showDebug?'Ocultar':'Mostrar'}</button>
        </div>
      </div>
      {showDebug&&(
        <div style={{maxHeight:260,overflowY:'auto',padding:8}}>
          {debugLines.length===0&&(
            <p style={{color:'#334',fontSize:12,textAlign:'center',padding:8}}>Aguardando diagnóstico...</p>
          )}
          {debugLines.map((l,i)=>(
            <div key={i} style={{
              display:'flex',gap:8,padding:'3px 4px',borderRadius:4,
              background:i===debugLines.length-1?'#0a1929':'transparent',
              marginBottom:2
            }}>
              <span style={{fontSize:14,flexShrink:0}}>{l.status}</span>
              <div style={{flex:1,minWidth:0}}>
                <span style={{
                  color: l.status==='✅'?'#00cc88':
                         l.status==='❌'?'#ff6666':
                         l.status==='⚠️'?'#ffcc44':'#7ac8e8',
                  fontSize:12
                }}>{l.msg}</span>
                {l.detail&&(
                  <div style={{
                    color:'#556',fontSize:11,marginTop:2,
                    whiteSpace:'pre-wrap',wordBreak:'break-all'
                  }}>{l.detail}</div>
                )}
              </div>
              <span style={{color:'#334',fontSize:10,flexShrink:0}}>{l.time}</span>
            </div>
          ))}
        </div>
      )}
  ...
  const Preloader=()=>loader?(
    <div style={{
      position:'fixed',inset:0,background:'rgba(2,12,20,0.93)',
      display:'flex',flexDirection:'column',alignItems:'center',
      justifyContent:'center',zIndex:9999,gap:16
    }}>
      <div style={{fontSize:48}}>🏛️</div>
      <p style={{color:'#7ac8e8',fontSize:14,letterSpacing:2}}>{loader.label}</p>
      <div style={{width:260,height:12,background:'#0a1929',borderRadius:8,
        border:'1px solid #1a3a55',overflow:'hidden'}}>
      ...
      <p style={{color:'#00c9ff',fontWeight:900,fontSize:20}}>{loader.pct}%</p>
    </div>
  ):null
  ...
  if(screen==='home') return(
    <div style={S.root}>
      ...
      <div style={S.card}>
        ...
        <p style={S.sub}>Polícia vs Ladrão · Online</p>
  ...
  if(screen==='join') return(
    <div style={S.root}>
      ...
      <div style={S.card}>
        ...
        <div style={{...S.sbar,...S.sok}}>🟢 Sala: <strong style={{letterSpacing:4}}>{roomCode}</strong></div>
        ...
        <div style={S.rolePicker}>
          <p style={{color:'#7ac8e8',fontSize:12,textAlign:'center',marginBottom:8}}>Escolha seu papel:</p>
    ...
    <div style={S.root}>
      ...
      <div style={{...S.card,width:480}}>
        ...
        <p style={{color:'#4a8aaa',fontSize:11}}>
          Envie o código <strong style={{color:'#00c9ff',letterSpacing:3}}>{roomCode}</strong> para seu filho
        </p>
        <div style={{display:'flex',gap:12,width:'100%'}}>
          <div style={S.teamBox}>
            <p style={{color:'#00C9FF',fontWeight:700,fontSize:13,marginBottom:8}}>👮 POLICIAIS ({cops.length})</p>
          ...
          <div style={S.teamBox}>
            <p style={{color:'#FF6B35',fontWeight:700,fontSize:13,marginBottom:8}}>🦹 LADRÕES ({thieves.length}/4)</p>
      ...
      <div style={{display:'flex',gap:12,alignItems:'flex-start',maxWidth:900,width:'100%'}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:10,width:178}}>
          ...
          <div style={{...S.panel,flex:1}}>
            <p style={S.pt}>💬 Chat</p>
            <div style={{height:80,overflowY:'auto',background:'#030b13',
...
const S={
  ...
  btnP:{width:'100%',padding:'11px 0',background:'linear-gradient(90deg,#0096c7,#00c9ff)',
  ...
  hud:{display:'flex',gap:12,alignItems:'center',flexWrap:'wrap',background:'#060f1a',
    border:'1px solid #0a2a3a',borderRadius:10,padding:'7px 14px',marginBottom:10,
    fontSize:13,width:'100%',maxWidth:900},
